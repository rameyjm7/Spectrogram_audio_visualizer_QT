...
void AudioInput::computeSpectrogramSlice2(AudioInput* audioInput)
{
  int N = 1024;             // transform length
  int nf = 6000;              // # freqs to fill in powerspec
...
//  audioInput->windowedAudioFrame[i] = audioInput->windowingFunction[i]*cos(i);
  }
...
void AudioInput::computeSpectrogramSlice(AudioInput* audioInput)
{
  ...
  for (int i = 0; i<N; ++i) {
    audioInput->windowedAudioFrame[i] = audioInput->windowingFunction[i]*audioInput->audioBuffer[mod(audioInput->bufferIndex-N+i, audioInput->bufferSizeSamples)];
//  audioInput->windowedAudioFrame[i] = audioInput->windowingFunction[i]*(cos(PI*2*1200*i));
  }
  /* execute the planned FFT */
  fftwf_execute(audioInput->fftPlan);
