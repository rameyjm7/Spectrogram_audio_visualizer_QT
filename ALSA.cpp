  ...
  while (!alsaInput->quit) {
    ...
    if (!alsaInput->pause) {
      ...
      for (int i = 0; i<n; i++) { // read audioBufferChunk into our buffer ai->b ...
        ...
        write_ptr = AudioInput::mod(alsaInput->bufferIndex+i, alsaInput->bufferSizeSamples); // wraps around
      ...
      alsaInput->bufferIndex = AudioInput::mod(alsaInput->bufferIndex+n, alsaInput->bufferSizeSamples);  // update index (in one go)
